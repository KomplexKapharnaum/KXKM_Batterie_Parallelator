 ...
 *
 * Arduino Library for accessing the INA2xx Family of power measurement
 * devices\n\n See main library header file "INA.h" for details and license
 * information
 *
 */
...
#include <Wire.h> ///< I2C Library definition
    ...
    defined(ESP8266) || defined(STM32F1)
...
inaDet::inaDet(inaEEPROM &inaEE) {
  ...
  current_LSB = (uint64_t)maxBusAmps * 1000000000 /
                32767;                    // Get the best possible LSB in nA
  ...
  case INA237:
    ...
    // shuntVoltage_LSB = INA237_SHUNT_VOLTAGE_ADCRANGE0_LSB; // start with
    // ADCRANGE0 (0-40.96mV)
    shuntVoltage_LSB =
        INA237_SHUNT_VOLTAGE_ADCRANGE1_LSB; // start with ADCRANGE1 (0-163.84mV)
    break;
  case INA260:
    busVoltageRegister = INA_BUS_VOLTAGE_REGISTER;
    shuntVoltageRegister =
        INA260_SHUNT_VOLTAGE_REGISTER; // Register not present
...
@details If called without a parameter or with a 0 value, then the constructor
does nothing, but if a value is passed then using EEPROM is disabled and each
INA-Device found has its data (inaEEPROM structure size) stored in a array
dynamically allocated during library instatiation here. If there is not enough
space then the pointer isn't init- ialized and the program will abort later on.
No error checking can be done here
...
INA_Class::~INA_Class() {
  ...
  @details If dynamic memory has been allocated for device storage rather than
  the default EEPROM, then that memory is freed here; otherwise the destructor
  does nothing
  */
  if (_expectedDevices) {
    delete[] _DeviceArray;
...
int16_t INA_Class::readWord(const uint8_t addr,
  ...
  /*! @brief     Read one word (2 bytes) from the specified I2C address
      @details   Standard I2C protocol is used, but a delay of I2C_DELAY
     microseconds has been added to let the INAxxx devices have sufficient time
     to get the return data ready
...
void INA_Class::readInafromEEPROM(const uint8_t deviceNumber) {
  /*! @brief     Read INA device information from EEPROM
      @details   Retrieve the stored information for a device from EEPROM. Since
     this method is private and access is controlled, no range error checking is
     performed
...
#if defined(__AVR__) || defined(CORE_TEENSY) || defined(ESP32) ||              \
    defined(ESP8266) || (__STM32F1__)
#ifdef __STM32F1__ // STM32F1 has no built-in EEPROM
    uint16_t e =
        deviceNumber * sizeof(inaEE);   // it uses flash memory to emulate
    uint16_t *ptr = (uint16_t *)&inaEE; // "EEPROM" calls are uint16_t type
    for (uint8_t n = sizeof(inaEE) + _EEPROM_offset; n;
         --n) // Implement EEPROM.get template
    {
      EEPROM.read(e++, ptr++); // for ina (inaDet type)
    }                          // of for-next each byte
#else
    EEPROM.get(_EEPROM_offset + (deviceNumber * sizeof(inaEE)),
               inaEE); // Read EEPROM values
#endif
#else
...
void INA_Class::writeInatoEEPROM(const uint8_t deviceNumber) {
  /*! @brief     Write INA device information to EEPROM
      @details   Write the stored information for a device from EEPROM. Since
     this method is private and access is controlled, no range error checking is
     performed
      @param[in] deviceNumber Index to device array */
  ...
  if (_expectedDevices == 0) {
    ...
    uint16_t e =
        deviceNumber * sizeof(inaEE); // it uses flash memory to emulate
    const uint16_t *ptr =
        (const uint16_t *)&inaEE; // "EEPROM" calls are uint16_t type
    for (uint8_t n = sizeof(inaEE) + _EEPROM_offset; n;
         --n) // Implement EEPROM.put template
    {
      EEPROM.update(e++, *ptr++); // for ina (inaDet type)
    }                             // for-next
#else
    EEPROM.put(_EEPROM_offset + (deviceNumber * sizeof(inaEE)),
               inaEE); // Write the structure
#ifdef ESP32
...
void INA_Class::setI2CSpeed(const uint32_t i2cSpeed) const {
  /*! @brief     Set a new I2C speed
      @details   I2C allows various bus speeds, see the enumerated type
     I2C_MODES for the standard speeds. The valid speeds are  100KHz, 400KHz,
     1MHz and 3.4MHz. Default to 100KHz when not specified. No range checking is
     done.
      @param[in] i2cSpeed [optional] changes the I2C speed to the rate specified
     in Herz */
...
uint8_t INA_Class::begin(const uint16_t maxBusAmps, const uint32_t microOhmR,
  ...
  /*! @brief     Initializes the contents of the class
      @details   Searches for possible devices and sets the INA Configuration
     details, without which meaningful readings cannot be made. If it is called
     without the optional deviceNumber parameter then the settings are applied
     to all devices, otherwise just that specific device is targeted. If the
     optional third parameter, devNo, is specified that specific device gets the
     two specified values set for it. Can be called multiple times, but the 3
     parameter version will only function after the 2 parameter version finds
     all devices.\n
      @param[in] maxBusAmps Integer value holding the maximum expected bus
     amperage, this value is used to compute a device's internal power register
      @param[in] microOhmR Shunt resistance in micro-ohms, this value is used to
     compute a device's internal power register
      @param[in] deviceNumber Device number to explicitly set the maxBusAmps and
     microOhmR values, by default all devices found get set to the same initial
     values for these 2 params
...
/***************************************************************************************************
** The AVR devices need to use EEPROM to save memory, some other devices have
*emulation for EEPROM**
** functionality while some devices have no such function calls. This library
*caters for these    **
** differences, with specialized calls for those platforms which have EEPROM
*calls and it makes   **
** the assumption that if the platform has no EEPROM call then it has sufficient
*RAM available at **
** runtime to allocate sufficient space for 32 devices. **
***************************************************************************************************/
#if defined(ESP32) || defined(ESP8266)
    EEPROM.begin(_EEPROM_size +
                 _EEPROM_offset); // If ESP32 then allocate 512 Bytes
    maxDevices =
        (_EEPROM_size) / sizeof(inaEE); // and compute number of devices
#elif defined(__STM32F1__)              // Emulated EEPROM for STM32F1
    maxDevices = (EEPROM.maxcount() - _EEPROM_offset) /
                 sizeof(inaEE); // Compute max possible
#elif defined(CORE_TEENSY)              // TEENSY doesn't have EEPROM.length
    maxDevices =
        (2048 - _EEPROM_offset) / sizeof(inaEE); // defined, so use 2Kb as value
#elif defined(__AVR__)
    maxDevices = (EEPROM.length() - _EEPROM_offset) /
                 sizeof(inaEE); // Compute max possible
#else
    maxDevices = 32;
#endif
         ...
         deviceAddress++) // Loop for each I2C addr
    {
      ...
      if (good == 0 &&
          _DeviceCount < maxDevices) // If no error and EEPROM has space
      {
        originalRegister = readWord(INA_CONFIGURATION_REGISTER,
                                    deviceAddress); // Save settings
        writeWord(INA_CONFIGURATION_REGISTER, INA_RESET_DEVICE,
                  deviceAddress); // Force reset
        tempRegister = readWord(INA_CONFIGURATION_REGISTER,
                                deviceAddress); // Read reset reg.
        if (tempRegister ==
            INA_RESET_DEVICE) // If the register wasn't reset then not an INA
        {
          writeWord(INA_CONFIGURATION_REGISTER, originalRegister,
                    deviceAddress); // restore value
        } else {
          if (tempRegister == 0x399F) // INA219
          {
            ...
            {
              tempRegister = readWord(INA_DIE_ID_REGISTER,
                                      deviceAddress); // Read the INA high-reg
              ...
              if (tempRegister == 0x6127) // INA260
              {
                ...
                if (tempRegister == 0x7127) //  INA3221
                {
                  ...
                  if (tempRegister == 0) {
                    tempRegister =
                        readWord(INA237_MANUFACTURER_ID_REGISTER,
                                 deviceAddress); // Read the manufacurer
                                                 // register on INA237
          ...
          if (inaEE.type != INA_UNKNOWN) // Increment device if valid INA2xx
          {
            ...
            inaEE.maxBusAmps = maxBusAmps > 1022
                                   ? 1022
                                   : maxBusAmps; // Clamp to maximum of 1022A
            ...
            if (inaEE.type == INA3221_0) {
              ...
              _DeviceCount = ((_DeviceCount + 1) %
                              maxDevices); // start again at 0 if overflow
    ...
    ina.maxBusAmps =
        maxBusAmps > 1022 ? 1022 : maxBusAmps; // Clamp to maximum of 1022A
...
void INA_Class::initDevice(const uint8_t deviceNumber) {
  /*! @brief     Initializes the the given devices using the settings from the
     internal structure
  ...
  if (ina.type == INA237) {
    ina.operatingMode =
        INA237_DEFAULT_OPERATING_MODE; // Default to continuous mode of INA237
  } else {
    ina.operatingMode =
        INA_DEFAULT_OPERATING_MODE; // Default to continuous mode
  }
  ...
  uint16_t calibration, maxShuntmV,
      tempRegister; // Calibration temporary variables
  switch (ina.type) {
  case INA219: // Set up INA219 or INA220
  {
    // Compute calibration register
    calibration =
        (uint64_t)409600000 / ((uint64_t)ina.current_LSB *
                               (uint64_t)ina.microOhmR / (uint64_t)100000);
    writeWord(INA_CALIBRATION_REGISTER, calibration,
              ina.address); // Write calibration
    /* Determine optimal programmable gain with maximum accuracy so no chance of
     * an overflow */
    maxShuntmV =
        ina.maxBusAmps * ina.microOhmR / 1000; // Compute maximum shunt mV
    if (maxShuntmV <= 40)
    ...
    tempRegister |= programmableGain
                    << INA219_PG_FIRST_BIT; // Overwrite the new values
    bitSet(tempRegister, INA219_BRNG_BIT);  // set to 1 for 0-32 volts
    writeWord(INA_CONFIGURATION_REGISTER, tempRegister,
              ina.address); // Write to config register
    break;
  }
  case INA226: {
  }
  ...
  case INA231: {
    ...
    // Compute calibration register
    calibration =
        (uint64_t)51200000 / ((uint64_t)ina.current_LSB *
                              (uint64_t)ina.microOhmR / (uint64_t)100000);
    writeWord(INA_CALIBRATION_REGISTER, calibration,
              ina.address); // Write calibration
    break;
  case INA237: {
    ...
    maxShuntmV =
        ina.maxBusAmps * ina.microOhmR / 1000; // Compute maximum shunt mV
    if (maxShuntmV <= 40) {                    // ADCRANGE=1 high resolution
      calibration = (4.0 * 819200000.0 * currentLSB_A * shuntR);
      writeWord(INA_CONFIGURATION_REGISTER, 0b10000,
                ina.address); // Select ADCRANGE1
    } else {                  // ADCRANGE=0 normal resolution
      calibration = (819200000.0 * currentLSB_A * shuntR);
    }
    writeWord(INA237_CALIBRATION_REGISTER, calibration,
              ina.address); // Write calibration
    break;
  }
  case INA260: {
...
void INA_Class::setBusConversion(const uint32_t convTime,
                                 const uint8_t deviceNumber) {
  /*! @brief     specifies the conversion rate in microseconds, rounded to the
     nearest valid value
      @details   INA devices can have a conversion rate of up to 68100
     microseconds
      @param[in] convTime The conversion time in microseconds, invalid values
     are rounded to the nearest valid value
      @param[in] deviceNumber [optional] When specified, only that specified
     device number gets changed, otherwise all devices are set to the same
     averaging rate
  */
        ...
        deviceNumber % _DeviceCount == i) // If device needs setting
    {
      ...
      case INA219:
        if (convTime >= 68100)
          ...
          convRate = 0;
        configRegister &=
            ~INA219_CONFIG_BADC_MASK;    // zero out the averages part
      ...
      case INA260:
        if (convTime >= 8244)
          ...
          convRate = 0;
        if (ina.type == INA226 || ina.type == INA3221_0 ||
            ina.type == INA3221_1 || ina.type == INA3221_2) {
          configRegister &=
              ~INA226_CONFIG_BADC_MASK;    // zero out the averages part
          configRegister |= convRate << 6; // shift in averages
        } else {
          configRegister &=
              ~INA260_CONFIG_BADC_MASK;    // zero out the averages part
...
void INA_Class::setShuntConversion(const uint32_t convTime,
  ...
  /*! @brief     specifies the conversion rate in microseconds, rounded to the
     nearest valid value
      @details   INA devices can have a conversion rate of up to 68100
     microseconds
      @param[in] convTime Conversion time in microseconds. Out-of-Range values
     are set to the closest valid value
      @param[in] deviceNumber to return the device name for[optional] When
     specified, only that specified device number gets changed, otherwise all
  ...
  for (uint8_t i = 0; i < _DeviceCount; i++) { // Loop for each device found
        ...
        deviceNumber % _DeviceCount == i) // If this device needs setting
    {
      ...
      configRegister = readWord(INA_CONFIGURATION_REGISTER,
                                ina.address); // Get register contents
      switch (ina.type) {
      case INA219:
        if (convTime >= 68100)
          convRate = 15;
          ...
          convRate = 0;
        configRegister &=
            ~INA219_CONFIG_SADC_MASK;    // zero out the averages part
      ...
      case INA260:
        ...
        if (ina.type == INA226 || ina.type == INA3221_0 ||
            ina.type == INA3221_1 || ina.type == INA3221_2) {
          configRegister &=
              ~INA226_CONFIG_SADC_MASK; // zero out the averages part
        } else {
          configRegister &=
              ~INA260_CONFIG_SADC_MASK; // zero out the averages part
...
const char *INA_Class::getDeviceName(const uint8_t deviceNumber) {
  /*! @brief     returns character buffer with the name of the device specified
     in the input param
...
uint8_t INA_Class::getDeviceAddress(const uint8_t deviceNumber) {
  /*! @brief     returns a I2C address of the device specified in the input
     parameter
      @details   Return the I2C address of the specified device, if number is
...
uint16_t INA_Class::getBusMilliVolts(const uint8_t deviceNumber) {
  /*! @brief     returns the bus voltage in millivolts
      @details   The converted millivolt value is returned and if the device is
     in triggered mode the next conversion is started
  ...
  busVoltage =
      (uint32_t)busVoltage * ina.busVoltage_LSB / 100; // conversion to get mV
  return (busVoltage);
...
uint16_t INA_Class::getBusRaw(const uint8_t deviceNumber) {
  /*! @brief     returns the raw unconverted bus voltage reading from the device
      @details   The raw measured value is returned and if the device is in
     triggered mode the next conversion is started
  ...
  uint16_t raw = readWord(ina.busVoltageRegister,
                          ina.address); // Get the raw value from register
  ...
  if (!bitRead(ina.operatingMode, 2) &&
      bitRead(ina.operatingMode, 1)) // Triggered & bus active
  {
    int16_t configRegister =
        readWord(INA_CONFIGURATION_REGISTER, ina.address); // Get current value
    writeWord(INA_CONFIGURATION_REGISTER, configRegister,
              ina.address); // Write to trigger next
...
int32_t INA_Class::getShuntMicroVolts(const uint8_t deviceNumber) {
  /*! @brief     returns the shunt reading converted to microvolts
      @details   The computed microvolts value is returned and if the device is
     in triggered mode the next conversion is started
  ...
  {
    int32_t busMicroAmps =
        getBusMicroAmps(deviceNumber); // Get the amps on the bus from device
    shuntVoltage = busMicroAmps / 200; // 2mOhm resistor, convert wiht Ohm's law
  } else {
    shuntVoltage =
        shuntVoltage * ina.shuntVoltage_LSB / 10; // Convert to microvolts
...
int16_t INA_Class::getShuntRaw(const uint8_t deviceNumber) {
  /*! @brief     Returns the raw shunt reading
      @details   The raw reading is returned and if the device is in triggered
     mode the next conversion is started
      @param[in] deviceNumber to return the value for
  ...
  {
    int32_t busMicroAmps =
        getBusMicroAmps(deviceNumber); // Get the amps on the bus
    raw = busMicroAmps / 200 / 1000;   // 2mOhm resistor, apply Ohm's law
  } else {
    raw = readWord(ina.shuntVoltageRegister,
                   ina.address); // Get the raw value from register
    if (ina.type == INA3221_0 || ina.type == INA3221_1 ||
        ina.type == INA3221_2) // Doesn't use 3 LSB
    {
      raw = raw >>
            3; // shift over 3 bits, datatype is "int" so shifts in sign bits
    }          // of if-then we need to shift INA3221 reading over
  }            // of if-then-else an INA260 with inbuilt shunt
  if (!bitRead(ina.operatingMode, 2) &&
      bitRead(ina.operatingMode, 0)) // Triggered & shunt active
  {
    int16_t configRegister =
        readWord(INA_CONFIGURATION_REGISTER, ina.address); // Get current reg
    writeWord(INA_CONFIGURATION_REGISTER, configRegister,
              ina.address); // Write to trigger next
  }                         // of if-then triggered mode enabled
  return (raw);
...
int32_t INA_Class::getBusMicroAmps(const uint8_t deviceNumber) {
  /*! @brief     Returns the computed microamps measured on the bus for the
     specified device
      @details   The computed reading is returned and if the device is in
     triggered mode the next conversion is started
      @param[in] deviceNumber to return the value for
  ...
  {
    microAmps = (int64_t)getShuntMicroVolts(deviceNumber) *
                ((int64_t)1000000 / (int64_t)ina.microOhmR);
  } else {
    microAmps = (int64_t)readWord(ina.currentRegister, ina.address) *
                (int64_t)ina.current_LSB / (int64_t)1000;
  } // of if-then-else an INA3221
  return (microAmps);
} // of method getBusMicroAmps()
int64_t INA_Class::getBusMicroWatts(const uint8_t deviceNumber) {
  /*!
  @brief     returns the computed microwatts measured on the bus for the
  specified device
  @details   The computed reading is returned and if the device is in triggered
  mode the next conversion is started
  @param[in] deviceNumber to return the value for
  ...
  */
  ...
  if (ina.type == INA3221_0 || ina.type == INA3221_1 ||
      ina.type == INA3221_2) // Doesn't compute Amps
  {
    microWatts = ((int64_t)getShuntMicroVolts(deviceNumber) * (int64_t)1000000 /
                  (int64_t)ina.microOhmR) *
                 (int64_t)getBusMilliVolts(deviceNumber) / (int64_t)1000;
  } else {
    microWatts = (int64_t)readWord(INA_POWER_REGISTER, ina.address) *
                 (int64_t)ina.power_LSB / (int64_t)1000;
  } // of if-then-else an INA3221
  return (microWatts);
...
void INA_Class::setMode(const uint8_t mode, const uint8_t deviceNumber) {
  /*!
  @brief     sets the operating mode from the list given in enum type "ina_Mode"
  for a device
  @details   If no device is specified, then all devices are set to the given
  mode
  @param[in] mode Mode (see "ina_Mode" enumerated type for list of valid values
  @param[in] deviceNumber to reset (Optional, when not set then all devices are
  mode changed)
  */
  int16_t configRegister;
  for (uint8_t i = 0; i < _DeviceCount; i++) // Loop for each device found
  {
        ...
        deviceNumber % _DeviceCount == i) // If this device needs setting
    {
      readInafromEEPROM(i); // Load EEPROM to ina structure
      configRegister = readWord(INA_CONFIGURATION_REGISTER,
                                ina.address);  // Get current config
...
bool INA_Class::conversionFinished(const uint8_t deviceNumber) {
  ...
  @details   The device's conversion ready bit is read and returned. "true"
  denotes finished conversion.
  @param[in] deviceNumber to check
  */
  if (_DeviceCount == 0)
    return false; // Return finished if invalid device. Issue #65
  readInafromEEPROM(deviceNumber %
                    _DeviceCount); // Load EEPROM to ina structure
  uint16_t cvBits = 0;
  switch (ina.type) {
  case INA219:
    cvBits = readWord(INA_BUS_VOLTAGE_REGISTER, ina.address) &
             2;                                // Bit 2 set denotes ready
...
void INA_Class::waitForConversion(const uint8_t deviceNumber) {
  /*!
  @brief     will not return until the conversion for the specified device is
  finished
  @details   if no device number is specified it will wait until all devices
  have finished their current conversion. If the conversion has completed
  already then the flag (and interrupt pin, if activated) is also reset.
  @param[in] deviceNumber to reset (Optional, when not set all devices have
  their mode changed)
  */
  uint16_t cvBits = 0;
  for (uint8_t i = 0; i < _DeviceCount; i++) // Loop for each device found
  {
        ...
        deviceNumber % _DeviceCount == i) // If this device needs setting
    {
      ...
      while (cvBits == 0) // Loop until the value is set
      {
        ...
        case INA260:
          cvBits =
              readWord(INA_MASK_ENABLE_REGISTER, ina.address) & (uint16_t)8;
          break;
        case INA3221_0:
        case INA3221_1:
        case INA3221_2:
...
bool INA_Class::alertOnConversion(const bool alertState,
                                  const uint8_t deviceNumber) {
  /*!
  @brief     configures the INA devices which support this functionality to pull
  the ALERT pin low when a conversion is complete
  @details   This call is ignored and returns false when called for an invalid
  device as the INA219 doesn't have this pin it won't work for that device.
  ...
  */
  ...
  {
    if (deviceNumber == UINT8_MAX ||
        deviceNumber == i) // If this device needs setting
    {
      readInafromEEPROM(i); // Load EEPROM to ina structure
      switch (ina.type) {
      case INA226:
      ...
      case INA260:
        alertRegister =
            readWord(INA_MASK_ENABLE_REGISTER, ina.address); // Get register
        alertRegister &= INA_ALERT_MASK; // Mask off all bits
        if (alertState)
          bitSet(alertRegister,
                 INA_ALERT_CONVERSION_RDY_BIT); // Turn on the bit
        writeWord(INA_MASK_ENABLE_REGISTER, alertRegister,
                  ina.address); // Write back
        returnCode = true;
        break;
      default:
        returnCode = false;
...
bool INA_Class::alertOnShuntOverVoltage(const bool alertState,
  ...
  /*!
  @brief     configures the INA devices which support this functionality to pull
  the ALERT pin low when the shunt current exceeds the value given in the
  parameter in millivolts
  @details   This call is ignored and returns false when called for an invalid
  device
  @param[in] alertState Boolean true or false to denote the requested setting
  ...
  */
  ...
  {
    if (deviceNumber == UINT8_MAX ||
        deviceNumber == i) // If this device needs to be processed
    {
      readInafromEEPROM(i); // Load EEPROM to ina structure
      switch (ina.type) {
      case INA226:
      case INA230:
      case INA231:
        alertRegister = readWord(INA_MASK_ENABLE_REGISTER,
                                 ina.address); // Get current register
        alertRegister &= INA_ALERT_MASK;       // Mask off all bits
        if (alertState) // If true, then also set threshold
        {
          bitSet(alertRegister,
                 INA_ALERT_SHUNT_OVER_VOLT_BIT); // Turn on the bit
          uint16_t threshold =
              milliVolts * 1000 / ina.shuntVoltage_LSB; // Compute using LSB
          writeWord(INA_ALERT_LIMIT_REGISTER, threshold,
                    ina.address); // Write register
        }                         // of if we are setting a value
        writeWord(INA_MASK_ENABLE_REGISTER, alertRegister,
                  ina.address); // Write register back
        returnCode = true;
        break;
      default:
        returnCode = false;
...
bool INA_Class::alertOnShuntUnderVoltage(const bool alertState,
  ...
  /*!
  @brief     configures the INA devices which support this functionality to pull
  the ALERT pin low when the shunt current goes below the value given in the
  parameter in millivolts
  @details   This call is ignored and returns false when called for an invalid
  device
  @param[in] alertState Boolean true or false to denote the requested setting
  ...
  for (uint8_t i = 0; i < _DeviceCount; i++) // Loop for each device found
  {
        ...
        deviceNumber % _DeviceCount == i) // If this device needs setting
    {
      ...
      case INA231:
        ...
        {
          bitSet(alertRegister,
                 INA_ALERT_SHUNT_UNDER_VOLT_BIT); // Turn on the bit
          uint16_t threshold =
              milliVolts * 1000 / ina.shuntVoltage_LSB; // Compute using LSB
          writeWord(INA_ALERT_LIMIT_REGISTER, threshold,
                    ina.address); // Write register
...
bool INA_Class::alertOnBusOverVoltage(const bool alertState,
  ...
  /*!
  @brief     configures the INA devices which support this functionality to pull
  the ALERT pin low when the bus current goes aboe the value given in the
  parameter in millivolts
  @details   This call is ignored and returns false when called for an invalid
  device
  @param[in] alertState Boolean true or false to denote the requested setting
  ...
  */
  ...
  for (uint8_t i = 0; i < _DeviceCount; i++) // Loop for each device found
  {
        ...
        deviceNumber % _DeviceCount == i) // If this device needs setting
    {
      ...
      case INA260:
        alertRegister = readWord(INA_MASK_ENABLE_REGISTER,
                                 ina.address); // Get the current register
        alertRegister &= INA_ALERT_MASK;       // Mask off all bits
        if (alertState)                        // Also set threshold
        {
          bitSet(alertRegister, INA_ALERT_BUS_OVER_VOLT_BIT); // Turn on the bit
          uint16_t threshold =
              milliVolts * 100 / ina.busVoltage_LSB; // Compute using LSB val
          writeWord(INA_ALERT_LIMIT_REGISTER, threshold,
                    ina.address); // Write register
        }                         // of if we are setting a value
        writeWord(INA_MASK_ENABLE_REGISTER, alertRegister,
...
bool INA_Class::alertOnBusUnderVoltage(const bool alertState,
  ...
  /*!
  @brief     configures the INA devices which support this functionality to pull
  the ALERT pin low when the bus current goes above the value given in the
  parameter in millivolts.
  @details   This call is ignored and returns false when called for an invalid
  device
  @param[in] alertState Boolean true or false to denote the requested setting
  @param[in] milliVolts alert level at which to trigger the alarm
  @param[in] deviceNumber to reset (Optional, when not set then all devices have
  their alert changed)
  @return    Returns "true" on success, otherwise false */
  uint16_t alertRegister;
  ...
  for (uint8_t i = 0; i < _DeviceCount; i++) // Loop for each device found
  {
        ...
        deviceNumber % _DeviceCount == i) // If this device needs setting
    {
      ...
      case INA260:
        ...
        {
          bitSet(alertRegister,
                 INA_ALERT_BUS_UNDER_VOLT_BIT); // Turn on the bit
          uint16_t threshold =
              milliVolts * 100 / ina.busVoltage_LSB; // Compute using LSB val
          writeWord(INA_ALERT_LIMIT_REGISTER, threshold,
                    ina.address); // Write register
...
bool INA_Class::alertOnPowerOverLimit(const bool alertState,
  ...
  /*!
  @brief     configures the INA devices which support this functionality to pull
  the ALERT pin low when the power exceeds the value set in the parameter in
  milliamps
  @details   This call is ignored and returns false when called for an invalid
  device
  @param[in] alertState Boolean true or false to denote the requested setting
  ...
  for (uint8_t i = 0; i < _DeviceCount; i++) // Loop for each device found
  {
        ...
        deviceNumber % _DeviceCount == i) // If this device needs setting
    {
      ...
      case INA260: // Devices with alert pin
        ...
        {
          bitSet(alertRegister,
                 INA_ALERT_POWER_OVER_WATT_BIT); // Turn on the bit
          uint16_t threshold =
              milliAmps * 1000000 / ina.power_LSB; // Compute using LSB val
          writeWord(INA_ALERT_LIMIT_REGISTER, threshold,
                    ina.address); // Write register
        }                         // of if we are setting a value
        writeWord(INA_MASK_ENABLE_REGISTER, alertRegister,
